...
describe('BookingsService', () => {
  ...
  describe('updateStatus', () => {
    ...
    it('should throw ConflictException on database exclusion constraint failure in create', async () => {
      const tomorrow = new Date();
      tomorrow.setDate(tomorrow.getDate() + 1);
      const dto: CreateBookingDto = {
        serviceId: 'service_1',
      ...
      const dbError = new QueryFailedError(
        'query',
        [],
        new Error('exclude_overlapping_bookings violation'),
      );
  ...
  describe('findOne', () => {
    ...
    it('should successfully return booking details to admin', async () => {
      const existing = {
        id: 'booking_1',
        clientId: 'client_1',
        service: { vendorId: 'vendor_1' } as unknown as Service,
    ...
    it('should throw ForbiddenException if user is not authorized to view the booking', async () => {
      ...
      bookingRepository.findOneById.mockResolvedValue(existing);
      await expect(
        service.findOne('booking_1', 'other_user', UserRole.CLIENT),
      ).rejects.toThrow(ForbiddenException);
    });
    ...
    it('should throw NotFoundException if booking does not exist', async () => {
      ...
      await expect(
        service.findOne('missing', 'client_1', UserRole.CLIENT),
