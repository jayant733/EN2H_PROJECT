...
describe('AuthService', () => {
  ...
  let userRepository: jest.Mocked<
    Pick<UserRepository, 'findOneByEmail' | 'findOneById' | 'save'>
  >;
  let sessionRepository: jest.Mocked<
    Pick<UserSessionRepository, 'save' | 'findOneActive'>
  >;
  let cryptoUtil: jest.Mocked<
    Pick<CryptoUtil, 'hashPassword' | 'comparePassword'>
  >;
  ...
  beforeEach(async () => {
    ...
    const mockConfigService = {
      get: jest.fn((key: string) => {
        if (key === 'auth.bcryptSaltRounds') return 12;
  ...
  describe('login', () => {
    ...
    it('should fail login if user is missing and waste CPU cycles', async () => {
      ...
      await expect(service.login(dto, '127.0.0.1', 'Mozilla')).rejects.toThrow(
  ...
  describe('refresh', () => {
    it('should rotate tokens successfully', async () => {
      ...
      sessionRepository.findOneActive.mockResolvedValue({
        id: 'session_id',
      ...
      expect(sessionRepository.save).toHaveBeenCalledTimes(2); // one for revoking, one for creating
    });
    ...
    it('should fail refresh if token signature is invalid', async () => {
      ...
      await expect(
        service.refresh('bad_token', '127.0.0.1', 'Mozilla'),
  ...
  describe('logout', () => {
    it('should revoke active session on logout', async () => {
      ...
      sessionRepository.findOneActive.mockResolvedValue(
        mockSession as unknown as UserSession,
      );
